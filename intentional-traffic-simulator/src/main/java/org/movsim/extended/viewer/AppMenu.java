...
package org.movsim.extended.viewer;
...
import org.movsim.viewer.ui.CanvasPanel;
import org.movsim.viewer.ui.MovSimMenuBase;
import org.movsim.viewer.ui.ViewerPreferences;
...
public class AppMenu extends MovSimMenuBase {
    ...
    public AppMenu(AppFrame mainFrame, Simulator simulator, CanvasPanel canvasPanel, TrafficCanvas trafficCanvas,
            ResourceBundle resourceBundle, Properties properties) {
    ...
    public AppMenu(AppFrameAgents mainFrame, Simulator simulator, CanvasPanel canvasPanel, TrafficCanvas trafficCanvas,
            ResourceBundle resourceBundle, Properties properties) {
        super(canvasPanel, trafficCanvas, resourceBundle);
        this.frame = mainFrame;
        this.simulator = simulator;
    ...
    private void uiDefaultReset() {
        ...
        if (frame instanceof AppFrame) {
            ((AppFrame)frame).statusPanel.setWithProgressBar(true);
            ((AppFrame)frame).statusPanel.reset();
        } else {
            ((AppFrameAgents)frame).statusPanel.setWithProgressBar(true);
            ((AppFrameAgents)frame).statusPanel.reset();
        }
        trafficCanvas.start();
    }
