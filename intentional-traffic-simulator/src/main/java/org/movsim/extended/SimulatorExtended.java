...
public class SimulatorExtended extends Simulator {
    ...
    private List<SimulationTimeStep> timeSteps;
    ...
    /**
     * <p>
     * Metodo para crear los semaforos de la red de tráfico. El metodo permite crear un proxy
     * para interceptar las llamadas y asociarlas a un agente BESA
     * </p>
     */
	private void createTrafficLights() {
		Enhancer enhTl = new Enhancer();

		enhTl.setSuperclass(TrafficLights.class);
		enhTl.setCallback(new TrafficLightsProxy());

		trafficLights = (TrafficLights) enhTl.create(new Class[] {
				org.movsim.autogen.TrafficLights.class, RoadNetwork.class },
				new Object[] { inputData.getScenario().getTrafficLights(),
						roadNetwork });
		
		trafficLights.getSignalIdToController();
	}
	
	/**
     * <p>
     * Metodo para crear los vehiculos. El metodo permite crear un proxy
     * para interceptar las llamadas y asociarlas a un agente BESA
     * </p>
     */
	private void createTrafficCompositionGenerator(Simulation simulationInput) {
		Enhancer enhTcg = new Enhancer();

		enhTcg.setSuperclass(TrafficCompositionGenerator.class);
		enhTcg.setCallback(new TrafficCompositionGeneratorProxy());

		defaultTrafficComposition = (TrafficCompositionGenerator) enhTcg
				.create(new Class[] { TrafficComposition.class,
						VehicleFactory.class },
						new Object[] { simulationInput.getTrafficComposition(),
								vehicleFactory });
	}
...
//        }
        ...
        for (SimulationTimeStep sTimeStep : this.timeSteps) {
        	sTimeStep.timeStep(dt, simulationTime, iterationCount);
		}
    }
	...
	public void addTimeStep(SimulationTimeStep simulationTimeStep, int index) {
		this.timeSteps.add(index, simulationTimeStep);
	}
}
