...
public class RoadSegment extends DefaultWeightedEdge implements Iterable<Vehicle> {
     ...
     */
    public synchronized void updateRoadConditions(double dt, double simulationTime, long iterationCount) {
        applySpeedLimits();
        applySlopes();
     ...
     *            the number of iterations that have been executed
     */
    public synchronized void makeLaneChanges(double dt, double simulationTime, long iterationCount) {
        if (laneCount < 2) {
            ...
            for (Iterator<Vehicle> vehIterator = laneSegment.iterator(); vehIterator.hasNext();) {
                ...
                synchronized (vehicle) {
                    ...
                    if (vehicle.considerLaneChange(dt, this)) {
                        ...
                        // Una vez serealiza el cambio de linea se reestablece a false
                        vehicle.setChangeLine(false);
                    }
                }
            }
        }
     ...
     *            the number of iterations that have been executed
     */
    public synchronized void updateVehicleAccelerations(double dt, double simulationTime, long iterationCount) {
        for (final LaneSegment laneSegment : laneSegments) {
     ...
     *            the number of iterations that have been executed
     */
    public synchronized void updateVehiclePositionsAndSpeeds(double dt, double simulationTime, long iterationCount) {
        for (final LaneSegment laneSegment : laneSegments) {
            assert laneSegment.laneIsSorted();
            for (final Vehicle vehicle : laneSegment) {
     ...
     *            the number of iterations that have been executed
     */
    public synchronized void outFlow(double dt, double simulationTime, long iterationCount) {
        for (final LaneSegment laneSegment : laneSegments) {
     ...
     *            the number of iterations that have been executed
     */
    public synchronized void inFlow(double dt, double simulationTime, long iterationCount) {
     ...
     *            the number of iterations that have been executed
     */
    public synchronized void updateDetectors(double dt, double simulationTime, long iterationCount) {
     ...
     */
    public synchronized void checkForInconsistencies(double time, long iterationCount, boolean isWithCrashExit) {
        for (final LaneSegment laneSegment : laneSegments) {
            int index = -1;
    ...
    public void setNode(NodeType nodeType, Long nodeId) {
        ...
        if (getNode(nodeType) != null && getNode(nodeType).longValue() != nodeId.longValue()) {
            throw new IllegalArgumentException("nodetype=" + nodeType.toString() + " of RoadSegment=" + userId()
                    + " already set=" + getNode(nodeType));
        }
